  ...
  <body>
    ...
    <div class="sadrzaj_pregled_bookmarka">
      <div class="new_bookmark_div">
        <a class="new_bookmark" href="bookmarks.php">Dodaj</a>
      </div>
      <?php
      if(isset($_GET['uspjesno']) && $_GET['uspjesno'] ==1){
        ...
        <div class="uspjeh">
          <p>
            <?php
            echo "Uspješno ste dodali novi bookmark.";
             ?>
          </p>
        </div>
        <?php
      }else if(isset($_GET['uspjesno']) && $_GET['uspjesno'] ==2){
        $poruka = "Netočan id bookmarka kojeg želite obrisati.";
      }else if(isset($_GET['uspjesno']) && $_GET['uspjesno'] ==3){
        ?>
        <div class="uspjeh">
          <p>
            <?php
            echo "Uspješno ste obrisali bookmark.";
             ?>
          </p>
        </div>
        <?php
      ...
      <div>
        ...
        <?php
        if(isset($_SESSION['email'])) $korisnik = $_SESSION['email'];
        $upit = "SELECT id_user from users WHERE email = '{$korisnik}'";
        $rezultat = $baza->select($upit);
        $podaci = mysqli_fetch_array($rezultat);
        $user_id = $podaci['id_user'];
        $upit = "SELECT * FROM bookmarks where id_user = '{$user_id}' order by redni_broj asc";
        $rezultat = $baza->select($upit);
        if($rezultat->num_rows == 0){
          echo "<div class='bookmark_error'>"
          . "<p>Niste dodali još ni jedan bookmark, dodajte ga klikom na gumb.</p>"
          . "</div>";
        }else{
          ?>
          ...
          <div class="bookmark_header">
            ...
            <p class="b_header_naziv">Naziv</p>
            <p class="b_header_opis">Opis</p>
          </div><br>
          <?php
          while($podaciBookmarka = $rezultat->fetch_array()){
            $redniBroj = $podaciBookmarka['redni_broj'];
            $naziv = $podaciBookmarka['naziv'];
            $opis = $podaciBookmarka['opis'];
            $id = $podaciBookmarka['id_bookmark'];
            echo "<div class='bookmark_row'><a href='bookmarks.php?id_bookmark={$id}'>"
            ...
            . $opis
            . "</p></a>"
            . "<a class='x' href='delete.php?id_bookmark={$id}'>&#10006;</a>"
            . "</div>";
          }
        }
